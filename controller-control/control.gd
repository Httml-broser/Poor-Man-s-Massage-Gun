...
# im cray cray baby idk how to make buttons via child nodes so code is the way to go
# good luck soldier
# second godot experience btw

var joy_array: Array[Array] = [[1,0.7], [0,1], [1,0.3]]
...
func create_dynamic_row_of_buttons(callback: Callable, count: int, button_height: int, wall_distance: int, spacing: int = 15) -> void:
	# Mostly Non Optional: Clear any existing buttons before creating new ones to prevent overlapping
	...
	var container_width: float = screen_size.x * 0.6
...
func _ready() -> void:
	...
	file_dialog.add_filter("*.joy, *.dat, *.txt", "Idk Text and shi")
...
func _process(delta: float) -> void:
	...
	if(Time.get_ticks_msec()>(asyncvar + update_vib)):
		asyncvar += update_vib
		var vib_temp: Array = [Input.get_joy_vibration_strength(joy_id).x,Input.get_joy_vibration_strength(joy_id).y]
		if !(vib_temp == [0,0]):
			Input.stop_joy_vibration(joy_id)
...
func apply_vibe(vib:Array, time:float = 0.0) -> void:
	# Weak motor (left), Strong motor (right), Duration (0 = infinite)
	Input.start_joy_vibration(joy_id, vib[0], vib[1], time)
	pass
...
func _refresh():
	delete_dynamic_buttons()
	create_dynamic_row_of_buttons(_on_button_pressed, joy_array.size(), 64, 15, 15)
	print("Refreshed!")
...
func save_to_file(content):
	...
	if file == null:
		...
		print("Path: ", default_file_path)
		return

	var data = var_to_str(content)
	file.store_string(data)
	file.close()

	print("Path: ", ProjectSettings.globalize_path(default_file_path))
	var a_
	if (content.size())<16:
		a_ = content
	else:
		a_ = "Too Big To Print 8="
		for i in range(roundi(content.size()/16)):
			a_ = a_ + "="
		a_ = a_ + "D"
	print("Data: ", a_)
...
func semihuman_unparcer() -> String:
	var output: String = str(round(precision_level*joy_array[0][0])/precision_level) + ":" + str(round(precision_level*joy_array[0][1])/precision_level)
	for i in range(1, joy_array.size()):
		output = output + " - " + str(round(precision_level*joy_array[i][0])/precision_level) + ":" + str(round(precision_level*joy_array[i][1])/precision_level)
	
	return output

func _update_joy_array(somthing: Array[Array]):
...
func try_sence():
	apply_vibe([slider1.value,slider2.value],5)
